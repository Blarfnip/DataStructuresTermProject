...
void PlayerQueue::enqueue(Player *playerIn, float priorityToAdd) { //priority only allows for low_priority queueing for players with penalty's and allows
    //allows line cutting if priority for a certain player is higher than 1 (default priority is 1)
    PlayerNode* newNode= new PlayerNode(playerIn,priorityToAdd);
    if(front= nullptr){
        front=newNode;
        back=newNode;
    }
        ...
        if(front->getPriority()==newNode->getPriority()){ //when priorities are the same it just acts like a normal queue
            ...
            throw std::out_of_range("Weird stuff went down");
        }
    }
}
